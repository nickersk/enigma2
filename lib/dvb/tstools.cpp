		...
		if (m_streaminfo.getStructureEntryFirst(local_offset, data) == 0)
		{
			for(int retries = 8; retries != 0; --retries)
			{
				if ((data & 0x1000000) != 0)
				{
					...
					eDebug("[eDVBTSTools] getPTS got it from sc file offset=%ld pts=%llu", local_offset, pts);
		...
		if (packet[3] & 0x20)
		{
			...
			if (packet[4])
			{
				if (packet[5] & 0x10) /* PCR present */
				{
					...
					offset -= 188;
					eDebug("[eDVBTSTools] getPTS PCR %16llx found at %ld pid %02x (%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x)", pts, offset, pid, packet[0], packet[1], packet[2], packet[3], packet[4], packet[5], packet[6], packet[7], packet[8], packet[9], packet[10]);
...
/*		if (m_pid >= 0)
		...
		if (payload[7] & 0x80) /* PTS */
		{
			...
			eDebug("[eDVBTSTools] getPTS PTS %16llx found at %ld pid %02x stream: %02x", pts, offset, pid, payload[3]);
	...
	else
	{
		...
		if (!m_samples.empty())
		{
			...
			while (maxtries--)
			{
				...
				eDebug("[eDVBTSTools] getOffset using: %llu:%llu -> %ld:%ld", l->first, u->first, l->second, u->second);
			...
			{
				pts = p;
				eDebug("[eDVBTSTools] getOffset aborting. Taking %ld as offset for %lld", offset, pts);
				return 0;
			}
		}
		...
		eDebug("[eDVBTSTools] getOffset fallback, bitrate=%d, results in %016lx", bitrate, offset);
		offset -= offset % m_packet_size;
		return 0;
	}
}
...
void eDVBTSTools::calcBegin()
{
	...
	if (!(m_begin_valid || m_futile))
	{
		...
		if (m_streaminfo.getFirstFrame(m_offset_begin, m_pts_begin) == 0)
		{
			...
			{
				eDebug("[eDVBTSTools] calcBegin [@ML] m_streaminfo.getLastFrame returned %ld, %llu (%us), fixup to: %ld, %llu (%us)",
...
void eDVBTSTools::takeSamples()
{
	...
	eDebug("[eDVBTSTools] takeSamples step %ld, pts begin %llu, pts end %llu, offs begin %ld, offs end %ld:",
	...
	if (!eDVBTSTools::getPTS(off, p, 1))
	{
		...
		if (l != m_samples.begin())
		{
			...
			if (u != m_samples.end())
			{
				...
				{
					eDebug("[eDVBTSTools] takeSample ignoring sample %ld %ld %ld (%llu %llu %llu)",
		...
		eDebug("[eDVBTSTools] takeSample adding sample %ld: pts %llu -> pos %ld (diff %ld bytes)", offset_org, p, off, off-offset_org);
		m_samples[p] = off;
		return 0;
	}
	return -1;
