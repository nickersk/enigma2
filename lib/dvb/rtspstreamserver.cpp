...
int eRTSPStreamClient::getOrbitalPosition(int fe, int src)
{
	...
	for (int i = 0; i < 144; i++)
	{
		...
		eDebug("%d: orbital position %d, m_slot_mask %d, lnNum %lu, old OP %d", i, op1, lnb_param.m_slot_mask, lnb_param.m_satellites.size(), lnb_param.SatCR_idx); //lnb_param.LNBNum, lnb_param.old_orbital_position);
		if (current_src >= src)
		{
...
{
	eDebug("%s: operation %d, pid_str %s (len pids: %lu)", __FUNCTION__, op, pid_str.c_str(), pids.size());
...
{
	eDebug("update pids called: %lu pids in the queue", pids.size());
